...
client.on("ready", () => {
  ...
  standup.getDataAndSchdule(database,client);
...
client.on("message", async (msg) => {
  ...
  // }
    ...
    msg.content.startsWith("!giverole") &&
    ...
    if (msg.member.roles.cache.some((role) => role.name === "team")) {
      ...
      const roleToAssign = msg.guild.roles.cache.find(
        (role) => role.id == batchID
      );
      for (var i = 0; i < studentIdList.length; i++) {
        var studentmember = msg.guild.members.cache.find(
          (member) => member.id == studentIdList[i]
    ...
    msg.content.startsWith("!createrole") &&
    msg.channel.name === "bot"
  ) {
    if (msg.member.roles.cache.some((role) => role.name === "team")) {
      ...
      msg.guild.roles.create({
        data: { name: roleName, permissions: rolePermissions },
      });
      msg.reply(
        "Role :" + roleName + "created with permissions :" + rolePermissions
      );
    }
  } else if (
    msg.content.startsWith("!createchannel") &&
    msg.channel.name === "bot"
  ) {
    if (msg.member.roles.cache.some((role) => role.name === "team")) {
      var splitMsgContents = msg.content.split(" "); // splitting command content
      ...
      msg.reply(
        "Channel :" + channelName + "created with reason :" + channelReason
      );
    }
